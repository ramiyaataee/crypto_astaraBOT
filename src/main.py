...
def validate_symbols():
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            valid_symbols = {s['symbol'] for s in data['symbols'] if s['status'] == 'TRADING'}
            invalid = [s for s in SYMBOLS if s not in valid_symbols]
            if invalid:
                logger.warning(f"⚠️ Invalid symbols in testnet: {invalid}")
            return invalid
        else:
            logger.error(f"❌ Failed to validate symbols: {response.status_code}")
            return SYMBOLS
    except Exception as e:
...
def ensure_csv_header():
    try:
        if not os.path.exists(CSV_FILE):
            ...
            logger.info(f"✅ Created CSV header: {CSV_FILE}")
    except Exception as e:
...
def append_csv_row(symbol, price, volume, change_percent):
    try:
        ...
        logger.info(f"✅ Appended CSV row for {symbol}")
    except Exception as e:
            ...
            async for message in ws:
                try:
                    ...
                    if not isinstance(data, dict) or data.get('e') != '24hrTicker':
                        logger.debug(f"Skipping non-ticker message: {data.get('e')}")
                        continue
...
async def watcher_loop():
    invalid_symbols = validate_symbols()
    valid_symbols = [s for s in SYMBOLS if s not in invalid_symbols]
    if not valid_symbols:
        logger.error("❌ No valid symbols available for testnet!")
        app_status['status'] = 'failed_no_valid_symbols'
        return
    logger.info(f"✅ Valid symbols: {valid_symbols}")
    uris = build_stream_path(valid_symbols)
    attempt = 0
    max_attempts = 50
